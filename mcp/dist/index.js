...
// ─── ORÇAMENTOS ──────────────────────────────────────────────────────────────
server.tool('list_orcamentos', 'Lista orçamentos da esteira comercial com filtros opcionais', {
    ...
    if (estrategico !== undefined)
        params.set('estrategico', String(estrategico));
    const data = await req('GET', `/orcamentos?${params}`);
    return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
});
server.tool('get_orcamento', 'Retorna detalhes completos de um orçamento incluindo histórico de alterações', { id: z.string().describe('UUID do orçamento') }, async ({ id }) => {
    const data = await req('GET', `/orcamentos/${id}`);
    return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
});
server.tool('create_orcamento', 'Cria um novo orçamento na esteira', {
...
server.tool('update_orcamento', 'Atualiza campos de um orçamento existente', {
...
server.tool('get_pipeline_orcamentos', 'Retorna orçamentos em ENVIADO, AGUARDANDO e APROVADO agrupados por probabilidade com totais R$', {}, async () => {
    const all = await req('GET', '/orcamentos?status=ENVIADO');
    const aguardando = await req('GET', '/orcamentos?status=AGUARDANDO');
    const aprovado = await req('GET', '/orcamentos?status=APROVADO');
...
// ─── OBRAS ───────────────────────────────────────────────────────────────────
server.tool('list_obras', 'Lista obras ativas com filtros opcionais', {
...
server.tool('get_obra', 'Retorna detalhes completos de uma obra', { id: z.string().describe('UUID da obra') }, async ({ id }) => {
    const data = await req('GET', `/obras/${id}`);
    return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
});
// ─── USUÁRIOS ────────────────────────────────────────────────────────────────
server.tool('list_users', 'Lista usuários ativos do BÈR App', {}, async () => {
