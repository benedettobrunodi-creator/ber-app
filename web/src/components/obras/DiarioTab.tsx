          ...
          {dataRetro === null ? (
            <>
              ...
              <button
                ...
                disabled={saving}
                className="rounded-lg border border-ber-gray/30 px-3 py-2 text-xs font-semibold text-ber-gray hover:text-ber-carbon hover:bg-ber-bg/40 disabled:opacity-60"
                title="Criar diário de um dia anterior"
              >
                Outro dia…
              </button>
            </>
            ...
            <>
              <input
                ...
                className="rounded-lg border border-ber-gray/30 px-2 py-1.5 text-xs"
              />
              <button
                onClick={() => dataRetro && criarDiario(dataRetro)}
                disabled={saving || !dataRetro}
                className="flex items-center gap-1.5 rounded-lg bg-ber-olive px-3 py-2 text-xs font-semibold text-white disabled:opacity-60"
              >
                {saving ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
                Criar
              </button>
              <button onClick={() => setDataRetro(null)} className="text-xs text-ber-gray hover:text-ber-carbon px-1">Cancelar</button>
            </>
          )}
        </div>
      </div>
