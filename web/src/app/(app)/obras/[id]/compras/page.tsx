                  ...
                  if (item.tipo === 'co') {
                    ...
                    return (
                      ...
                      {item.splits.map(sp => sp.coTipo !== null ? (
                        ...
                        (() => {
                          const spMeta = sp.valor * (1 - sp.pctMeta);
                          const spSavOrç = sp.valor - sp.comprado;
                          const spSavMeta = spMeta - sp.comprado;
                          ...
                          return (
                        <tr key={sp.id} className={`border-l-2 ${sp.coTipo === 'credito' ? 'bg-green-50/60 border-green-400' : 'bg-red-50/60 border-red-400'} ${sp.compradoOk ? 'opacity-60' : ''}`}>
                          <td className="px-3 py-1.5 text-center">
                            <input type="checkbox" checked={sp.compradoOk}
                          ...
                          <td colSpan={2} className="px-3 py-1.5">
                            <input type="text" value={sp.descricao || ''}
                          ...
                          <td className="px-3 py-1.5 text-center">
                            <input type="number" min={0} max={100} step={1}
                              value={Math.round(sp.pctMeta * 100)}
                          ...
                          <td className="px-3 py-1.5">
                            <input {...valorInputProps(`splitc_${sp.id}`, sp.comprado, v => saveSplit(item.id, sp.id, { comprado: v }))}
                              placeholder="0"
                              className={`w-full rounded border px-1 py-0.5 text-right text-xs tabular-nums focus:outline-none ${borderCol} ${bgCol}`} />
                          </td>
                          <td className="px-3 py-1.5">
